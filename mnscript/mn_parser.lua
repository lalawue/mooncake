...
do
	local __lib__ = require("mn_utils")
	fSet, fSplit, fTrim = __lib__.set, __lib__.split, __lib__.trim
end
fSetmaxstack(10240)
-- remove keyword pattern
local kwTableR = fSet({ "and", "break", "case", "class", "continue", "default", "defer", "do", "else", "elseif", "end", "export", "fn", "for", "from", "function", "goto", "if", "import", "in", "local", "not", "or", "repeat", "return", "switch", "then", "until", "while", "...", ".." })
...
-- MARK: grammar
local vFile, vShebang, vFnSMT, vClsDef, vClsBody, vClsAsSMT, vClsFnDef, vTbDef, vTbKV, vTbNk, vTbVk, vTbBk, vTbVal, vFnDefKW, vFnDefArg, vFnDefVArg, vFnDefArgs, vFnDefBody, vFnDefName, vFnDefOnly, vFnCall, vImSMT, vImLib, vImRight, vExSMT, vExLeft, vAsSMT, vAsLeft, vAsRight, vAswSMT, vIfSMT, vElseIfSMT, vElseSMT, vSwSMT, vSwCaSMT, vSwDeSMT, vForSMT, vForVar, vForNext, vForList, vRptSMT, vWhileSMT, vBrkSMT, vCntSMT, vGoSMT, vRetSMT, vDeferSMT, vBlkSMT, vCmSMT, vDotExpr, vParaExpr, vSquareExpr, vColonExpr, vEvalExpr, vEvalLeft, vClsOpUnit, vClsExpr, vOpUnit, vExpr = V("vFile"), V("vShebang"), V("vFnSMT"), V("vClsDef"), V("vClsBody"), V("vClsAsSMT"), V("vClsFnDef"), V("vTbDef"), V("vTbKV"), V("vTbNk"), V("vTbVk"), V("vTbBk"), V("vTbVal"), V("vFnDefKW"), V("vFnDefArg"), V("vFnDefVArg"), V("vFnDefArgs"), V("vFnDefBody"), V("vFnDefName"), V("vFnDefOnly"), V("vFnCall"), V("vImSMT"), V("vImLib"), V("vImRight"), V("vExSMT"), V("vExLeft"), V("vAsSMT"), V("vAsLeft"), V("vAsRight"), V("vAswSMT"), V("vIfSMT"), V("vElseIfSMT"), V("vElseSMT"), V("vSwSMT"), V("vSwCaSMT"), V("vSwDeSMT"), V("vForSMT"), V("vForVar"), V("vForNext"), V("vForList"), V("vRptSMT"), V("vWhileSMT"), V("vBrkSMT"), V("vCntSMT"), V("vGoSMT"), V("vRetSMT"), V("vDeferSMT"), V("vBlkSMT"), V("vCmSMT"), V("vDotExpr"), V("vParaExpr"), V("vSquareExpr"), V("vColonExpr"), V("vEvalExpr"), V("vEvalLeft"), V("vClsOpUnit"), V("vClsExpr"), V("vOpUnit"), V("vExpr")
local gG = { vFile, vFile = vShebang ^  - 1 * vFnSMT ^ 0, vFnSMT = (vBrkSMT + vCntSMT + vCmSMT + vClsDef + vFnDefName + vIfSMT + vSwSMT + vForSMT + vRptSMT + vWhileSMT + vRetSMT + vGoSMT + vAsSMT + vFnCall + vAswSMT + vExSMT + vImSMT + vDeferSMT + vBlkSMT) * Cmt(pSemi * pBlanks / fTrim, fMarkStype(";")) ^  - 1, --
...
vFnDefKW = pBlanks * Cg(P("fn"), "stype") * pBlanks, vFnDefArg = pBlanks * Cg(pVarArg) * pBlanks, vFnDefVArg = pBlanks * Cg(pVArgList) * pBlanks, vFnDefArgs = Ct((vFnDefArg * pComma) ^ 0 * vFnDefVArg + vFnDefArg * (pComma * vFnDefArg) ^ 0), vFnDefBody = pBLeft * Ct(vFnSMT ^ 0) * pBRight, vFnDefName = pBlanks * Ct(Cg(P("local") + P("export"), "attr") ^  - 1 * vFnDefKW * Cg(pVarLeft, "name") * pPLeft * Cg(vFnDefArgs, "args") ^ 0 * pPRight * Cg(vFnDefBody, "body")), vFnDefOnly = pBlanks * Ct(Cg(P("fn"), "etype") * pPLeft * Cg(vFnDefArgs, "args") ^ 0 * pPRight * Cg(vFnDefBody, "body")), --
vFnCall = pBlanks * Ct(Cg(fName("("), "stype") * (pVarArg + pCPLeft * vExpr * pCPRight) * ((vDotExpr + vSquareExpr) ^ 0 * (vColonExpr + vParaExpr)) ^ 1), --
vIfSMT = pBlanks * Ct(Cg(P("if"), "stype") * pBlanks * Ct(vExpr) * pBLeft * Ct(vFnSMT ^ 0)) * vElseIfSMT ^ 0 * vElseSMT ^  - 1 * Cmt(pBRight, fMarkStype("ifend")), vElseIfSMT = pBRight * Ct(Cg(P("elseif"), "stype") * pBlanks * Ct(vExpr) * pBLeft * Ct(vFnSMT ^ 0)), vElseSMT = pBRight * Ct(Cg(P("else"), "stype") * pBLeft * Ct(vFnSMT ^ 0)), --
vSwSMT = pBlanks * Ct(Cg(P("switch"), "stype") * pBlanks * Ct(vExpr) * pBlanks * pBLeft * vSwCaSMT ^ 1 * vSwDeSMT ^  - 1 * pBRight), vSwCaSMT = pBlanks * Ct(Cg(P("case"), "stype") * pBlanks * Ct(vExpr) * (pBlanks * pComma * pBlanks * Ct(vExpr)) ^ 0 * pBlanks * pColon * Cg(Ct(vFnSMT ^ 0), "body")), vSwDeSMT = pBlanks * Ct(Cg(P("default"), "stype") * pBlanks * pColon * Cg(Ct(vFnSMT ^ 0), "body")), --
vForSMT = pBlanks * Ct(Cg(P("for"), "stype") * Ct(vForVar * vForNext + vForList) * pBLeft * Ct(vFnSMT ^ 0) * pBRight), vForVar = pBlanks * Cg(pVarArg) * pBlanks * Cg(pEqual, "sub") * pBlanks * Ct(vExpr) * pBlanks * pComma, vForNext = pBlanks * Ct(vExpr) * pBlanks * (pComma * pBlanks * Ct(vExpr)) ^  - 1 * pBlanks, vForList = pBlanks * Cg(pVarArg) * pBlanks * (pComma * pBlanks * Cg(pVarArg)) ^ 0 * pBlanks * Cg(P("in"), "sub") * pBlanks * Ct(Ct(vExpr) * pBlanks * (pComma * Ct(vExpr)) ^  - 2) * pBlanks, --
vRptSMT = pBlanks * Ct(Cg(P("repeat"), "stype") * pBLeft * Ct(vFnSMT ^ 0) * pBRight * P("until") * pBlanks * Ct(vExpr)) * pBlanks, vWhileSMT = pBlanks * Ct(Cg(P("while"), "stype") * pBlanks * Ct(vExpr) * pBlanks * pBLeft * Ct(vFnSMT ^ 0) * pBRight), --
...
vRetSMT = pBlanks * Ct(Cg("return", "stype") * (pBlanks * Ct(vExpr) * pBlanks * (pComma * pBlanks * Ct(vExpr) * pBlanks) ^ 0) ^ 0) * pBlanks * ( # pBRight +  # P("case") +  # P("default") +  - 1), vDeferSMT = pBlanks * Ct(Cg("defer", "stype") * Cg(Cp(), "pos") * pBLeft * Ct(vFnSMT ^ 0)) * pBRight, vBlkSMT = pBlanks * Ct(Cg("{", "stype") * Ct(vFnSMT ^ 1) * pBRight) * pBlanks, --
...
vOpUnit = pBlanks * (pSingleOp * vOpUnit + vTbDef + vFnDefOnly + pStr + pNumber + pVArgList + vEvalExpr) * pBlanks, vExpr = pCPLeft * vExpr * pCPRight * (pOperator * vExpr + vParaExpr) ^ 0 + vOpUnit * (pOperator * vExpr) ^ 0 }
